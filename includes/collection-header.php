...
<div id="coll-intro" class="grid_12"> 
	...
	<?php if ($topic->post_title): ?>
		...
		<ul class="meta-gestures">
		    <li class="twitter"> 
		        <a href="<?php echo esc_url( 'http://twitter.com/share?url=' . $topic->guid . '&text=' ) . argo_get_twitter_title(); ?>" class="twitter-share-button" data-count="horizontal">Tweet</a>
		    </li>
		    <li class="fb">
		        <a name="fb_share" share_url="<?php echo esc_url( $topic->guid ); ?>" type="button_count" href="<?php echo esc_url( 'http://www.facebook.com/sharer.php?u=' . $topic->guid . '&t=' ) . get_the_title();  ?>">Share</a>
		    </li>
		</ul>	
	<?php else: ?>
